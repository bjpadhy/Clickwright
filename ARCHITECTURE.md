...
**Determinism:** `temperature 0` and JSON mode (`response_format: json_object`) on the OpenAI-compatible path; Anthropic rejects a non-default temperature, so its effort stays pinned at `medium`. `seed` and `reasoning_effort` are sent only when set — Gemini 400s on unknown fields.
